...
namespace ManagedDoom
{
    public sealed class Wad : IDisposable
    {
        ...
        /// <summary>
        /// Load a WAD directly from an in-memory stream (no filesystem access).
        /// The caller must keep the stream alive for the lifetime of this Wad instance.
        /// wadName must match the WAD type for game mode detection: "doom1", "doom2", etc.
        /// </summary>
        public Wad(Stream stream, string wadName)
        {
            try
            {
                Console.Write("Open WAD from memory: ");

                names = new List<string>();
                streams = new List<Stream>();
                lumpInfos = new List<LumpInfo>();

                AddStream(stream, wadName);
        ...
        private void AddFile(string fileName)
        {
            ...
            AddStream(stream, Path.GetFileNameWithoutExtension(fileName));
        }
